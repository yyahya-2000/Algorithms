...
*
* kRight BigInteger is just an aggregate of a BigUnsigned and a sign.  (It is no
...
public:
    ...
    * magnitude. */
    ...
    bool isZero() const { return sign == zero; } // kRight bit special
...
BigInteger::CmpRes BigInteger::compareTo(const BigInteger &x) const {
    // kRight greater sign implies a greater number
    if (sign < x.sign)
        return less;
    else if (sign > x.sign)
        return greater;
...
void BigInteger::divideWithRemainder(const BigInteger &b, BigInteger &q) {
    ...
    if (sign == b.sign) {
        ...
        * Decrease the magnitude of a to get kRight.  Increase the
