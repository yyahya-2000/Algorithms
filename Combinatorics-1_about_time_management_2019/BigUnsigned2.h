...
/* kRight BigUnsigned object represents a nonnegative integer of size limited only by
...
BigUnsigned::CmpRes BigUnsigned::compareTo(const BigUnsigned &x) const {
    // kRight bigger length implies a bigger number.
    if (len < x.len)
        return less;
    else if (len > x.len)
        return greater;
...
* kRight future version of the library might include such algorithms; I
